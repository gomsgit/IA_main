...
from .models import upload_image,upload_category
...
# def sample_select(request):
#     if request.method == 'POST':
#         print("entering into bottle defect select")
#         bot = request.POST.get('email')
...
#     return render(request, "not correct")

def file_exist(i,name,cat):
    import os
    current_path = os.getcwd()
    print("current_path", current_path)
    sp_path = "\\".join(current_path.split("\\"))
    print("sp_path", sp_path)
    final_path = sp_path+"\\"+"media"+"\\"+name+"-"+cat+"\\"
    print("finllllll",final_path+str(i))
    res = os.path.exists(final_path + str(i))
    print(res)
    return res
@api_view(['POST'])
...
def image_upload(request):
    ...
    if request.method == "POST":
        ...
        if posts_serializer.is_valid():
            table_value=posts_serializer.data
            print("valid",table_value)

            ans = upload_category(name=table_value["name"],category=table_value["category"])
            ans.save()

            for i in files:
                res = file_exist(i,table_value["name"],table_value["category"])
                print("reasssssssssss", res)
                if res!=True:
                    print("if condition")
                    file_instance = upload_image(images=i, name=table_value["name"],
                                                        category=table_value["category"],upload_time=dt_string)
                    file_instance.save()
                else:
...
def size_folder():
    ...
    sp_path = "\\".join(current_path.split("\\"))
    print("sp_path", sp_path)
    path = sp_path + "\\" + "media" + "\\" + name + "-" + cat + "\\"
    print("finllllll", path)
    total_size = 0
    ...
    mb = str(total_size / (1024 * 1024))+"MB"
    print("tolll", mb)
    return mb
@api_view(['POST','GET'])
...
def batch_detail(request):
    ...
    if request.method == "GET":
        ...
        table_data = upload_image.objects.all()
        print("table_data",table_data)
        file_data = PostSerializer(table_data,many=True)

        data_li=file_data.data
        print("no of files",len(data_li))
        image_li=[]
        import os
        for i in data_li:
            i_val=list(i.values())
            i_val[1]=i_val[1]+"-"+i_val[2]
            file_name = os.path.basename(i_val[3])
            file_name = "".join(os.path.splitext(file_name))

            image_li.append(file_name)

        print("imm",image_li)
        final_dict={"batch_name":i_val[1],"images":image_li,"upload_time":i_val[4],
                    "no_of_files":len(data_li),"size_of_folder":size_folder()}
...
def category_list(request):
    ...
    if request.method == "GET":
        ...
        table_data = upload_category.objects.values("name","category")
        print("table_data",table_data)
        final_val=(list(table_data))
